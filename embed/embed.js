/**
 * CA Bot — Embed Script v8
 * Floating AI chatbot with bounce, tooltip, slide-left, slide-back loop.
...
// Animation cycle: 3 bounces → show "Need Help?" on RIGHT → slide LEFT → slide BACK → hide → loop (5s delay)
function startAnim(){
    animTimer=setTimeout(function cycle(){
        ...
        setTimeout(function(){
            if(!animating)return;
            // After bounce: show tooltip on RIGHT side of bubble
            tt2.classList.add('visible');
            bg2.classList.add('slide');
            setTimeout(function(){
                if(!animating)return;
                // After showing: slide back right, hide tooltip
                bg2.classList.remove('slide');
                tt2.classList.remove('visible');
                btn.classList.remove('bounce');
                // Loop with 5s delay
                animTimer=setTimeout(cycle,5000);
            },2000); // 2s visible pause
        },1800); // 3 bounces * 0.6s = 1.8s
