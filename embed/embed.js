...
// Tooltip at right:40px — fully visible AFTER bubble slides left (to right:96).
// During bounce, tooltip is hidden (opacity:0). Only appears after slide completes.
...
// Tooltip is SEPARATE at right:40px. Only appears AFTER bubble slides left (to right:96).
...
function startAnim(){
    animTimer=setTimeout(function cycle(){
        ...
        setTimeout(function(){
            ...
            setTimeout(function(){
                if(!animating)return;
                tt2.classList.add('visible'); // appears at right:40 — fully to the RIGHT of slid bubble
                setTimeout(function(){
                    if(!animating)return;
            ...
            },500); // slight delay after slide for smoothness
        },1800);
    },2000);
}
